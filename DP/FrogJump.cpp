...
The frog wants to reach the 'Nth' stair. 'HEIGHT[i]' is the height of the '(i+1)th' stair.If Frog jumps from 'ith' to 'jth' stair, the energy lost in the jump is given by absolute value of ( HEIGHT[i-1] - HEIGHT[j-1] ).
...
int frogDp(int index,vector<int> &heights,vector<int> &dp){
    ...
    int right=INT_MAX;
    if(index>1){
        right=frogDp(index-2,heights,dp)+abs(heights[index]-heights[index-2]);
    }
